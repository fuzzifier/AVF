function [NMI, MI] = nmi(true_labels, cluster_labels)
...
% Sum up n_ij*log()
index = find(product > 0);
product(index) = log2(product(index));
product = cmat .* product;
score = sum(product(:));
MI = score/n;
% Divide by sqrt( sum(n_i*log(n_i/n)) * sum(n_j*log(n_j/n)) )
index = find(n_i > 0);
n_i(index) = n_i(index) .* log2(n_i(index)/n);
index = find(n_j > 0);
n_j(index) = n_j(index) .* log2(n_j(index)/n);
